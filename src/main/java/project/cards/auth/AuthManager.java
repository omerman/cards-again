...
public class AuthManager extends BusModBase {
            ...
            public void handle(Message<JsonObject> reply) {
                ...
                if (reply.body().getString("status").equals("ok")) {
                    if (reply.body().getObject("result") != null) {
                        ...
                        final String sessionID;
                        if (info != null) {//Found and already logged in.
                            sessionID = info.sessionID;
                        }
                        else {//Found not logged in.
                            sessionID = UUID.randomUUID().toString();
                            long timerID = doTimer(sessionID,username);
                        ...
                        vertx.eventBus().publish("user.register",new JsonObject().putString("sessionID", sessionID).putString("username",username));
                    } else {
                        // Not found
    ...
    protected void doAuthorise(Message<JsonObject> message) {
        ...
        if (username != null) {
            ...
            extendTimer(sessionID,username);
    ...
    private long doTimer(final String sessionID,final String username) {
        return vertx.setTimer(sessionTimeout, new Handler<Long>() {
            public void handle(Long timerID) {
                sessions.remove(sessionID);
                logins.remove(username);
                vertx.eventBus().send("user.unregister", new JsonObject().putString("sessionID", sessionID));
            }
        });
    }

    private void extendTimer(String sessionID,String username) {

        logger.info("extendTimer for: "+username);

        LoginInfo info = logins.get(username);
        vertx.cancelTimer(info.timerID);

        logins.put(username,new LoginInfo(doTimer(sessionID,username),sessionID));
    }
